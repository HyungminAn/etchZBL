...
for i in range(len(rxn_list)):
    ...
    rxn_E[i,0]+=np.loadtxt(f"post_process_bulk_gas/{idx[0]}/{rxn_list[i][1][0]}/bpnn/e")
    if len(rxn_list[i][1])>1:
        for j in rxn_list[i][1][1:]:
            if j in trivial_list:
                rxn_E[i,0]+=np.loadtxt(f"post_process_bulk_gas/gas/trivial/{j}/bpnn/e")
            else:
                idx=j.split("_")
                rxn_E[i,0]+=np.loadtxt(f"post_process_bulk_gas/gas/{idx[0]}/{j}/bpnn/e")
    ...
    rxn_E[i,0]-=np.loadtxt(f"post_process_bulk_gas/{idx[0]}/{rxn_list[i][0][0]}/bpnn/e")
...
class atom_image():
    ...
    def cal_test_num_neigh(self,images):
        ...
        for e,i in enumerate(images):
            atomic_nums=i.get_atomic_numbers()
            NN_list=ase.neighborlist.neighbor_list('ij',i,self.bond_length_dict)
            num_neighs[e]=4*np.sum(atomic_nums==1)+3*np.sum(atomic_nums==2)+np.sum(atomic_nums==3)+np.sum(atomic_nums==4)-len(NN_list[0])
            # Si_list=np.nonzero(atomic_nums==1)[0]
...
idx=np.argsort(-rxn_E[:,0])
print("#BPNN_rxn gnn_rxn error coord_change rxn")
for i in idx:
