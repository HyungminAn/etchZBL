...
PairZBLp::PairZBLp(LAMMPS *lmp) : Pair(lmp) {
  writedata = 1;
}
...
PairZBLp::~PairZBLp()
{
  ...
  if (allocated) {
    ...
    memory->destroy(outer_dist);
    memory->destroy(inner_dist);
    memory->destroy(outer_sq);
    memory->destroy(inner_sq);
...
void PairZBLp::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      // Debug
      //printf("i %i j  %i \n global_sq %f compare %f\n",itype,jtype, cut_globalsq, outer_sq[itype][jtype]);
      //exit(1);

      //if (rsq < cut_globalsq) {
      if (rsq < outer_sq[itype][jtype]) {

        //printf("i %i j  %i : global_sq %f compare %f\n",itype,jtype, cut_globalsq, outer_sq[itype][jtype]);
        r = sqrt(rsq);
        fpair = dzbldr(r, itype, jtype);

        //if (rsq > cut_innersq) {
        if (rsq > inner_sq[itype][jtype]) {
          //t = r - cut_inner;
          t = r - inner_dist[itype][jtype];
          fswitch = t*t *
...
void PairZBLp::allocate()
{
  ...
  //Allocate array for pair distance
  memory->create(outer_dist,n+1,n+1,"pair:outer_dist");
  memory->create(inner_dist,n+1,n+1,"pair:inner_dist");
  memory->create(outer_sq,n+1,n+1,"pair:outer_sq");
  memory->create(inner_sq,n+1,n+1,"pair:inner_sq");
...
void PairZBLp::settings(int narg, char **arg)
{
...
void PairZBLp::coeff(int narg, char **arg)
{
  double z_one, z_two;
  double inner, outer;
...
//    error->all(FLERR,"Incorrect args for pair coefficients");
  ...
  // For inner outer params
  inner = utils::numeric(FLERR,arg[4],false,lmp);
  outer = utils::numeric(FLERR,arg[5],false,lmp);
  //
    ...
    for (int j = MAX(jlo,i); j <= jhi; j++) {
      if (i == j) {
        if (z_one != z_two)
          error->all(FLERR,"Incorrect args for pair coefficients");
        z[i] = z_one;
        // Parameter for same atom type
        outer_dist[i][i] = outer ; 
        inner_dist[i][i] = inner ; 

        outer_sq[i][i] = outer * outer; 
        inner_sq[i][i] = inner * inner; 
      }
      else {
        //Parameter for inner outer distance
        outer_dist[i][j] = outer ; 
        inner_dist[i][j] = inner ; 
        outer_dist[j][i] = outer ; 
        inner_dist[j][i] = inner ; 

        outer_sq[i][j] = outer * outer; 
        inner_sq[i][j] = inner * inner; 
        outer_sq[j][i] = outer * outer; 
        inner_sq[j][i] = inner * inner; 
      ...
      set_coeff(i, j, z_one, z_two, inner, outer);
      count++;
    }
  }
...
double PairZBLp::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
    //set_coeff(i, j, z[i], z[j]);
    set_coeff(i, j, z[i], z[j], inner_dist[i][j], outer_dist[i][j]);

  //return cut_global;
  return outer_dist[i][j] ;
}
...
void PairZBLp::write_restart(FILE *fp)
{
...
void PairZBLp::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++) {
    ...
    for (j = 1; j <= atom->ntypes; j++){
        MPI_Bcast(&inner_dist[i][j],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&outer_dist[i][j],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&inner_dist[j][i],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&outer_dist[j][i],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&inner_sq[i][j],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&outer_sq[i][j],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&inner_sq[j][i],1,MPI_DOUBLE,0,world);
        MPI_Bcast(&outer_sq[j][i],1,MPI_DOUBLE,0,world);
...
void PairZBLp::write_restart_settings(FILE *fp)
{
...
void PairZBLp::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairZBLp::write_data(FILE *fp)
{
...
void PairZBLp::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairZBLp::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
...
double PairZBLp::e_zbl(double r, int i, int j) {
...
double PairZBLp::dzbldr(double r, int i, int j) {
...
double PairZBLp::d2zbldr2(double r, int i, int j) {
...
//void PairZBLt::set_coeff(int i, int j, double zi, double zj)
void PairZBLp::set_coeff(int i, int j, double zi, double zj, double inner, double outer)
{
...
*/
  ...
  double tc = outer - inner;
  double fc = e_zbl(outer, i, j);
  double fcp = dzbldr(outer, i, j);
  double fcpp = d2zbldr2(outer, i, j);
