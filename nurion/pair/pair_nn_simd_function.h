...
#ifdef __AVX512F__  
#ifdef __AVX512VL__
    // AVX-512 with AVX-512VL (original case)
    return _mm512_i32gather_pd(_mm256_loadu_epi32(vindex), base, 8);
#else
    // AVX-512 without AVX-512VL (e.g., KNL)
    const int v_size = MM512_DOUBLE_LEN;
    double result[v_size];
    for (int i = 0; i < v_size; ++i) {
        result[i] = base[vindex[i]];
    }
    return _mm512_loadu_pd(result);
#endif
#else
#ifdef __AVX2__
