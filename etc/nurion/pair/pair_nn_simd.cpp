...
inline void PairNNIntel::RadialVector_simd(const int ielem, const int jelem, const int ii_idx, const int jj_idx,const int stride_nsym, const double rRij, const double* vecij, double* symvec, double* tmpf)
{
  ...
  for (int s=0; s < vector_size; s+=SIMD_V_LEN) {
    ...
    */
    ...
    for (int my_loop_i = 0; my_loop_i < SIMD_V_LEN; my_loop_i++) {
        int base_idx = tt_offset + s + my_loop_i;
        symvec[base_idx] += G2_v[my_loop_i];
        tmpf[SIMD_V_LEN*(jj_idx + 0 * stride_nsym) + base_idx] += deriv_v[my_loop_i] * vecij[0];
        tmpf[SIMD_V_LEN*(jj_idx + 1 * stride_nsym) + base_idx] += deriv_v[my_loop_i] * vecij[1];
        tmpf[SIMD_V_LEN*(jj_idx + 2 * stride_nsym) + base_idx] += deriv_v[my_loop_i] * vecij[2];

        tmpf[SIMD_V_LEN*(ii_idx + 0 * stride_nsym) + base_idx] -= deriv_v[my_loop_i] * vecij[0];
        tmpf[SIMD_V_LEN*(ii_idx + 1 * stride_nsym) + base_idx] -= deriv_v[my_loop_i] * vecij[1];
        tmpf[SIMD_V_LEN*(ii_idx + 2 * stride_nsym) + base_idx] -= deriv_v[my_loop_i] * vecij[2];
    }
  }
}
...
inline void PairNNIntel::AngularVector1_simd_AVX2(const int ielem, const int jelem, const int kelem, const int ii_idx, const int jj_idx, const int kk_idx, const int stride_nsym, const double rRij, const double rRik, const double rRjk, const double* vecij, const double* vecik, const double* vecjk, const double* precal_ang, double* symvec, double* tmpf) {
  ...
  for (int s=0; s < vector_size; s+=SIMD_V_LEN) {
    ...
    */
    for (int my_loop_i = 0; my_loop_i < SIMD_V_LEN; my_loop_i++) {
        symvec[tt_offset+s+my_loop_i] += G4_v[my_loop_i];
    }
    simd_v tmpd_ij_x = deriv_ij*vecij[0];
    simd_v tmpd_ij_y = deriv_ij*vecij[1];
    simd_v tmpd_ij_z = deriv_ij*vecij[2];
    ...
    */
    for (int my_loop_i = 0; my_loop_i < SIMD_V_LEN; my_loop_i++) {
        ...
        tmpf[SIMD_V_LEN*(jj_idx + 0 * stride_nsym) + base_idx] += tmpd_ij_x[my_loop_i] - tmpd_jk_x[my_loop_i];
        tmpf[SIMD_V_LEN*(jj_idx + 1 * stride_nsym) + base_idx] += tmpd_ij_y[my_loop_i] - tmpd_jk_y[my_loop_i];
        tmpf[SIMD_V_LEN*(jj_idx + 2 * stride_nsym) + base_idx] += tmpd_ij_z[my_loop_i] - tmpd_jk_z[my_loop_i];

        tmpf[SIMD_V_LEN*(kk_idx + 0 * stride_nsym) + base_idx] += tmpd_ik_x[my_loop_i] + tmpd_jk_x[my_loop_i];
        tmpf[SIMD_V_LEN*(kk_idx + 1 * stride_nsym) + base_idx] += tmpd_ik_y[my_loop_i] + tmpd_jk_y[my_loop_i];
        tmpf[SIMD_V_LEN*(kk_idx + 2 * stride_nsym) + base_idx] += tmpd_ik_z[my_loop_i] + tmpd_jk_z[my_loop_i];

        tmpf[SIMD_V_LEN*(ii_idx + 0 * stride_nsym) + base_idx] -= tmpd_ij_x[my_loop_i] + tmpd_ik_x[my_loop_i];
        tmpf[SIMD_V_LEN*(ii_idx + 1 * stride_nsym) + base_idx] -= tmpd_ij_y[my_loop_i] + tmpd_ik_y[my_loop_i];
        tmpf[SIMD_V_LEN*(ii_idx + 2 * stride_nsym) + base_idx] -= tmpd_ij_z[my_loop_i] + tmpd_ik_z[my_loop_i]; 
    }
  }
}
